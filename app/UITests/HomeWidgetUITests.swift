// CI: приложение импортирует тестовую гифку (аргумент --ci-import-test), затем виджет
// «Animated Widget» ставится на домашний экран. Анимация не выбрана — виджет показывает
// последнюю сохранённую (так задумано и для пользователя).
...
final class HomeWidgetUITests: XCTestCase {
    private let springboard = XCUIApplication(bundleIdentifier: "com.apple.springboard")

    func testImportAndAddWidget() throws {
        // Приложение открыть один раз, иначе виджета нет в галерее
        let app = XCUIApplication()
        app.launchArguments = ["--ci-import-test"]
        app.launch()
        // импорт тестовой гифки и публикация в App Group
        let done = app.staticTexts["ci-import-done"]
        XCTAssertTrue(done.waitForExistence(timeout: 60), "импорт тестовой гифки не завершился")
        sleep(2)
        ...
        guard let result = firstExisting([
            springboard.cells["WidgetLab"],
            springboard.buttons["WidgetLab"],
            springboard.staticTexts["WidgetLab"],
        ], timeout: 10) else {
            XCTFail(dump("нет WidgetLab в результатах поиска"))
            return
        }
        result.tap()
        sleep(2)
